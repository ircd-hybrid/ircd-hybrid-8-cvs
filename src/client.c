 ...
 *  USA
 *
 *  $Id: client.c,v 1.9 2002/04/27 05:30:24 a1kmm Exp $
 */
...
*/
/* int exit_client(client_p, source_p, from,comment)
 * Input: client_p: The local client from which the request for the client to
                    be exited originated from. May be NULL if this was caused
                    by this server for internal reasons.
 *        source_p: The client which is being exited
 *        from: The source of this exit. Use &me if this server generated it.
 *        comment: The reason why they are being exited.
 * Output: Returns -1 if client_p equals source_p, otherwise 0.
 * Side-effects: The client is marked as dead and removed from all lists it may
 *               be on. It is added to the unknown list. If it is a server all
 *               dependent servers and clients are exited. If the user was not
 *               killed, the exit message is propagated.
 */
int
exit_client(struct Client *client_p, struct Client *source_p,
            struct Client *from, const char *comment)
{
  ...
  if (IsServer(source_p))
  {
    if (ConfigServerHide.hide_servers)
      ...
      ircsprintf(comment1, "%s %s", source_p->servptr->name, source_p->name);
