 ...
 *  USA
 *
 *   $Id: s_serv.c,v 1.8 2002/04/27 05:30:26 a1kmm Exp $
 */
...
sendnick_TS(struct Client *client_p, struct Client *target_p)
{
  ...
  if (HasID(target_p) && IsCapable(client_p, CAP_UID))
    sendto_one(client_p, "CLIENT %s %d %lu %s %s %s %s %s :%s",
               ...
               target_p->servptr->name, target_p->user->id, target_p->info);
  else
...
fork_server(struct Client *server)
{
  ...
  else if (ret == 0)
  {
    ...
    /* network read/write */
    sprintf(fd_str[4], "%d", server->localClient->fd);
...
serv_connect(struct ConfItem *aconf, struct Client *by)
{
  ...
  {
    comm_connect_tcp(client_p->localClient->fd, aconf->host, aconf->port, NULL,
                     0, serv_connect_callback, client_p, aconf->aftype, 30);
  }
                ...
                char *reason, char *client_reason)
{
   ...
   * defined in the call.
   */
  if (!IsDead(client_p) && client_reason != NULL)
  {
